...
///// check for unnatural values, set to 0 not to check
$unnatural = 0;
...
while($row = mysql_fetch_array($query)) 
  { 
    ...
    $temp0[$i] = $row['temp0'];
    ///// check for unnatural data
    if (abs($temp0[$i]-$temp0[$i-1]) > 20 && $i > 0 && unnatural >= 1) {
      $temp0[$i] = $temp0[$i-1];
    }
    $temp1[$i] = $row['temp1'];
    ///// check for unnatural data
    if (abs($temp1[$i]-$temp1[$i-1]) > 20 && $i > 0 && unnatural >= 1) {
      $temp1[$i] = $temp1[$i-1];
    }
